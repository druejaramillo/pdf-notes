...
It uses Mathpix for PDF-to-Markdown conversion, then preserves LaTex inline math (`\(...\)`) and display math (`\[...\]`), converts LaTex sections to Markdown headings, and turns each regular line into a bullet. Sentences separated by `. ` become nested bullets, matching the original tool's formatting convention.
...
- Inline and display math stay in Mathpix's LaTex delimiters so Obsidian can render them.
